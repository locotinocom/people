...
export function useReward(
  ...
  containerRef?: React.RefObject<HTMLDivElement> // optional, aber empfohlen
) {
  ...
  const xpGen = useRef(0)

  /** ---- XP ---- */
  const spawnXp = (amount: number, durationOverride?: number) => {
    if (amount <= 0 || !xpTargetRef.current) return Promise.resolve()

    // Startpunkt = Container-Mitte (oder Fallback: Bildschirmmitte)
    ...
    // Zielpunkt = Mitte des Ziel-Elements
    ...
    xpGen.current += 1
    const myGen = xpGen.current
    setXpParticles(particles)

    const computedTotalMs = (count - 1) * 50 + 1200
    const totalMs = durationOverride ?? computedTotalMs

    xpInFlight.current = new Promise<void>((resolve) => {
      window.setTimeout(() => {
        if (xpGen.current === myGen) {
          setXpParticles([])
  ...
  const RewardParticles = () => (
    <div className="fixed inset-0 pointer-events-none z-50">
      {xpParticles.map((p) => (
        ...
        >
          <span className="px-1 py-[1px] rounded bg-yellow-300/90 text-zinc-900">
            xp
          </span>
        </motion.span>
      ))}
