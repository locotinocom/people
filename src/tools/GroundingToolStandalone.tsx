// src/tools/GroundingToolStandalone.tsx
// Standalone-Version der Verankerungsübung – für den Tools-Screen
...
function BodyScanStandalone({ onRestart }: { onRestart: () => void }) {
    ...
    return (
      ...
      >
        ...
        <motion.button
          onClick={onRestart}
          whileTap={{ scale: 0.97 }}
          className="w-full max-w-xs py-3 rounded-2xl border border-teal-700 text-teal-400 hover:bg-teal-900/30 font-semibold transition-all"
        >
          Nochmal
        </motion.button>
      </motion.div>
  ...
  return (
    <AnimatePresence mode="wait">
      ...
      >
          ...
          {BODY_SCAN_STEPS.map((s, i) => (
            <div
              ...
              className={clsx(
                ...
                i < stepIndex ? "bg-teal-400" : i === stepIndex ? "bg-white scale-125" : "bg-white/20"
              )}
            />
          ))}
        </div>

        <LightOrb emoji={currentStep.emoji} glowColor="rgba(20,184,166,0.5)" progress={progress} />
        ...
        <p className="mt-6 text-2xl font-light text-white/50 tabular-nums">{timeLeft}s</p>
...
function LightMeditationStandalone({ onRestart }: { onRestart: () => void }) {
  const [stepIndex, setStepIndex] = useState(0)
    ...
    return (
      ...
      >
        ...
        <motion.button
          onClick={onRestart}
          whileTap={{ scale: 0.97 }}
          className="w-full max-w-xs py-3 rounded-2xl border border-violet-700 text-violet-400 hover:bg-violet-900/30 font-semibold transition-all"
        >
          Nochmal
        </motion.button>
      </motion.div>
  ...
  return (
    <AnimatePresence mode="wait">
      ...
      >
          ...
          {LIGHT_MEDITATION_STEPS.map((s, i) => (
            <div
              ...
              className={clsx(
                ...
                i < stepIndex ? "bg-white/60" : i === stepIndex ? "bg-white scale-150" : "bg-white/15"
              )}
            />
          ))}
        </div>

        <LightOrb emoji={currentStep.emoji} glowColor={currentStep.glowColor} progress={progress} />
        ...
        <p className="mt-6 text-2xl font-light text-white/40 tabular-nums">{timeLeft}s</p>
...
export default function GroundingToolStandalone() {
  const [mode, setMode] = useState<"conservative" | "spiritual" | null>(null)
  ...
  if (!mode) {
    return (
      ...
      >
        ...
        <h2 className="text-2xl font-bold mb-2 text-center">Verankerung</h2>
        <p className="text-gray-400 text-sm mb-8 text-center max-w-xs">
          Für Momente, wenn der Kopf zu laut wird.<br />
  ...
  if (mode === "conservative") {
    return <BodyScanStandalone key={`body-${key}`} onRestart={handleRestart} />
  }

  return <LightMeditationStandalone key={`light-${key}`} onRestart={handleRestart} />
}
