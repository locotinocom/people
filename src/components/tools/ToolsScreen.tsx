...
import { selectPraxisItems } from "@store/slices/praxisSlice"
import ToolCard from "./ToolCard"
import PraxisScreen from "@components/praxis/PraxisScreen"
...
import HelpSupportTool from "@tools/HelpSupportTool"
import RelapseToolStandalone from "@tools/RelapseToolStandalone"
...
const TOOL_COMPONENTS: Record<string, React.ReactNode> = {
  ...
  safety_net_tool: <HelpSupportTool />,
  relapse_tool: <RelapseToolStandalone />,
}
...
export default function ToolsScreen() {
  ...
  const [activeTab, setActiveTab] = useState<"tools" | "praxis">("tools")
  ...
  // Zeige Praxis-Tab wenn das System freigeschaltet ist (egal ob aktuell Items vorhanden sind)
  // Ab Praxis-System-Level können neue Items spawn
  const systemUnlocked = true // TODO: Check gegen User-Level aus Backend

  return (
    <div className="w-full h-full flex flex-col bg-gray-900 text-white overflow-hidden">
      {/* Header & Tabs */}
      <div className="shrink-0 border-b border-white/10">
        <div className="w-full max-w-2xl mx-auto px-5 pt-5">
          <h2 className="text-xl font-bold text-white">🛠 Tools & Aufgaben</h2>
          <p className="text-sm text-white/50 mt-0.5">
            Hilfsmittel und Praxis-Aufgaben für deinen Alltag
          </p>
      ...
      <div className="flex-1 overflow-hidden">
        ...
        {/* Praxis Tab */}
        {activeTab === "praxis" && systemUnlocked && <PraxisScreen />}
      </div>
    </div>
  )
}
